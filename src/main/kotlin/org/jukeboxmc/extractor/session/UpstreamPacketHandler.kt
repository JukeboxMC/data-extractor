...
class UpstreamPacketHandler(private val dataExtractor: DataExtractor) : BedrockPacketHandler {
    ...
    override fun handle(packet: LoginPacket): PacketSignal {
        ...
        val loginChain = this.dataExtractor.msaAuth().generateLoginChain((keyPair.public as ECPublicKey), this.dataExtractor.codec().minecraftVersion)
        ...
        packet.extra = ForgeryUtil.forgeSkin(keyPair, packet.extra, "${this.dataExtractor.address().address.hostAddress}:${this.dataExtractor.address().port}", authExtraData.displayName)
